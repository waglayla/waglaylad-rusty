...
use waglayla_core::{info, waglaylad_env::version, time::unix_now, warn};
use waglayla_grpc_client::{ClientPool, GrpcClient};
use waglayla_notify::subscription::context::SubscriptionContext;
use waglayla_rpc_core::{api::rpc::RpcApi, notify::mode::NotificationMode};
...
impl Args {
    fn parse() -> Self {
        ...
        Args {
            ...
            rpc_server: m.get_one::<String>("rpcserver").cloned().unwrap_or("localhost:12110".to_owned()),
...
#[tokio::main]
async fn main() {
    waglayla_core::log::init_logger(None, "");
    let args = Args::parse();
    ...
    let schnorr_key = if let Some(private_key_hex) = args.private_key {
        ...
        let waglayla_addr = Address::new(ADDRESS_PREFIX, ADDRESS_VERSION, &pk.x_only_public_key().0.serialize());
        info!(
            "Generated private key {} and address {}. Send some funds to this address and rerun rothschild with `--private-key {}`",
            sk.display_secret(),
            String::from(&waglayla_addr),
            sk.display_secret()
        );
        return;
    };

    let waglayla_addr = Address::new(ADDRESS_PREFIX, ADDRESS_VERSION, &schnorr_key.x_only_public_key().0.serialize());

    rayon::ThreadPoolBuilder::new().num_threads(args.threads as usize).build_global().unwrap();

    info!("Using Rothschild with private key {} and address {}", schnorr_key.display_secret(), String::from(&waglayla_addr));
...
\tMedian time: {}, \n\tDAA score: {}, \n\tPruning point: {}, \n\tTips: {}, \n\t{} virtual parents: ...{}, \n\tCoinbase maturity: {}",
    ...
    let mut utxos = refresh_utxos(&rpc_client, waglayla_addr.clone(), &mut pending, coinbase_maturity).await;
    ...
    loop {
        ...
        if !has_funds || now - last_refresh > 60_000 {
            ...
            utxos = refresh_utxos(&rpc_client, waglayla_addr.clone(), &mut pending, coinbase_maturity).await;
...
async fn refresh_utxos(
    ...
    waglayla_addr: Address,
    pending: &mut HashMap<TransactionOutpoint, u64>,
    coinbase_maturity: u64,
) -> Vec<(TransactionOutpoint, UtxoEntry)> {
    populate_pending_outpoints_from_mempool(rpc_client, waglayla_addr.clone(), pending).await;
    fetch_spendable_utxos(rpc_client, waglayla_addr, coinbase_maturity, pending).await
}
...
async fn populate_pending_outpoints_from_mempool(
    ...
    let entries = rpc_client.get_mempool_entries_by_addresses(vec![waglayla_addr], true, false).await.unwrap();
    let now = unix_now();
...
async fn fetch_spendable_utxos(
    ...
    let resp = rpc_client.get_utxos_by_addresses(vec![waglayla_addr]).await.unwrap();
    ...
    let txs = selected_utxos_groups
        ...
        .map(|utxo_option| {
            if let Some((selected_utxos, selected_amount)) = utxo_option {
                let tx = generate_tx(schnorr_key, &selected_utxos, selected_amount, num_outs, &waglayla_addr);
...
fn generate_tx(
    ...
    let script_public_key = pay_to_address_script(waglayla_addr);
    let inputs = utxos
        .iter()
