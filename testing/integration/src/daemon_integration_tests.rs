...
use waglayla_consensus::params::SIMNET_PARAMS;
use waglayla_consensusmanager::ConsensusManager;
use waglayla_core::{task::runtime::AsyncRuntime, trace};
use waglayla_grpc_client::GrpcClient;
use waglayla_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use waglayla_rpc_core::{api::rpc::RpcApi, Notification, RpcTransactionId};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  waglayla_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut waglaylad1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = waglaylad1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut waglaylad2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = waglaylad2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut waglaylad1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut waglaylad2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = waglaylad1.start().await;
    let rpc_client2 = waglaylad2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", waglaylad1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(waglaylad1.network.into(), waglayla_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2.get_virtual_chain_from_block(waglayla_consensus::params::SIMNET_GENESIS.hash, true).await.unwrap();
...
/// `cargo test --release --package waglayla-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("waglayla-testing-integration-heap.json").build();

    waglayla_core::log::try_init_logger(
        "INFO,waglayla_testing_integration=trace,waglayla_notify=debug,waglayla_rpc_core=debug,waglayla_grpc_client=debug",
    );
    ...
    // Connect waglaylad2 to waglaylad1
    rpc_client2.add_peer(format!("127.0.0.1:{}", waglaylad1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    let miner_address =
        Address::new(waglaylad1.network.into(), waglayla_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(waglaylad1.network.into(), waglayla_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(waglaylad1.network.into(), waglayla_addresses::Version::PubKey, &[0; 32]);
    ...
    let mut clients = vec![ListeningClient::connect(&waglaylad2).await, ListeningClient::connect(&waglaylad1).await];
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    waglayla_core::log::try_init_logger("info,waglayla_grpc_core=trace,waglayla_grpc_server=trace,waglayla_grpc_client=trace,waglayla_core=trace");
    ...
    {
        let total_fd_limit = 10;
        let mut waglaylad1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = waglaylad1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = waglaylad1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&waglaylad1.core);
