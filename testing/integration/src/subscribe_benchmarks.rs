...
use waglayla_core::{info, task::tick::TickService, trace};
use waglayla_math::Uint256;
use waglayla_notify::scope::VirtualDaaScoreChangedScope;
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| Address::new((*network_id).into(), waglayla_addresses::Version::PubKey, &Uint256::from_u64(x as u64).to_le_bytes()))
        .collect_vec()
}

/// `cargo test --package waglayla-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    waglayla_core::log::try_init_logger(
        "INFO, waglayla_core::time=debug, waglayla_rpc_core=debug, waglayla_grpc_client=debug, waglayla_notify=info, waglayla_notify::address::tracker=debug, waglayla_notify::listener=debug, waglayla_notify::subscription::single=debug, waglayla_mining::monitor=debug, waglayla_testing_integration::subscribe_benchmarks=trace",
    );
    ...
    // Fold-up
    //
    waglayla_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    waglayla_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    waglayla_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package waglayla-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=16610 --p2p=16611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    waglayla_core::log::try_init_logger(
        "INFO, waglayla_core::core=trace, waglayla_core::time=debug, waglayla_rpc_core=debug, waglayla_grpc_client=debug, waglayla_notify=info, waglayla_notify::address::tracker=debug, waglayla_notify::listener=debug, waglayla_notify::subscription::single=debug, waglayla_mining::monitor=debug, waglayla_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    waglayla_core::panic::configure_panic();
...
/// `cargo test --package waglayla-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package waglayla-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package waglayla-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package waglayla-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package waglayla-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
