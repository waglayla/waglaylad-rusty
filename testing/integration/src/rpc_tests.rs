...
use waglayla_consensus::params::SIMNET_GENESIS;
use waglayla_consensus_core::{constants::MAX_SOMPI, subnets::SubnetworkId, tx::Transaction};
use waglayla_core::info;
use waglayla_grpc_core::ops::WaglayladPayloadOps;
...
use waglayla_rpc_core::{api::rpc::RpcApi, model::*, Notification};
use waglayla_utils::{fd_budget, networking::ContextualNetAddress};
...
/// `cargo test --release --package waglayla-testing-integration --lib -- rpc_tests::sanity_test`
#[tokio::test]
async fn sanity_test() {
    ...
    for op in WaglayladPayloadOps::list() {
        let network_id = daemon.network;
        let task: JoinHandle<()> = match op {
            WaglayladPayloadOps::SubmitBlock => {
            ...
            WaglayladPayloadOps::GetBlockTemplate => {
                tst!(op, "see SubmitBlock")
            }

            WaglayladPayloadOps::GetCurrentNetwork => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetInfo => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_info_call(GetInfoRequest {}).await.unwrap();
                    assert_eq!(response.server_version, waglayla_core::waglaylad_env::version().to_string());
            ...
            WaglayladPayloadOps::GetMempoolEntries => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            WaglayladPayloadOps::GetConnectedPeerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::Unban => {
                tst!(op, "see Ban")
            }

            WaglayladPayloadOps::SubmitTransaction => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetSubnetwork => {
                let rpc_client = client.clone();
                tst!(op, {
                    let result =
            ...
            WaglayladPayloadOps::GetVirtualChainFromBlock => {
                tst!(op, "see SubmitBlock")
            }

            WaglayladPayloadOps::GetBlockCount => {
                tst!(op, "see SubmitBlock")
            }

            WaglayladPayloadOps::GetBlockDagInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::ResolveFinalityConflict => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            WaglayladPayloadOps::GetHeaders => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            WaglayladPayloadOps::GetUtxosByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetBalanceByAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            WaglayladPayloadOps::GetBalancesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetSinkBlueScore => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sink_blue_score_call(GetSinkBlueScoreRequest {}).await.unwrap();
            ...
            WaglayladPayloadOps::EstimateNetworkHashesPerSecond => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            WaglayladPayloadOps::GetMempoolEntriesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::PubKey, &[0u8; 32])];
                    let response = rpc_client
            ...
            WaglayladPayloadOps::GetCoinSupply => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetMetrics => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetServerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetSyncStatus => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::GetDaaScoreTimestampEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifyBlockAdded => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifyNewBlockTemplate => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifyFinalityConflict => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifyUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifySinkBlueScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifyPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifyVirtualDaaScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::NotifyVirtualChainChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
                    rpc_client
            ...
            WaglayladPayloadOps::StopNotifyingUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            WaglayladPayloadOps::StopNotifyingPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
