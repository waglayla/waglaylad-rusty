...
use waglayla_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use waglayla_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use waglayla_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use waglayla_database::prelude::{DirectWriter, MemoryWriter};
use waglayla_database::prelude::{ReadLock, StoreError};
use waglayla_database::prelude::{StoreResult, DB};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; waglayla_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use waglayla_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(waglayla_database::prelude::ConnBuilder::default().with_files_limit(10));
