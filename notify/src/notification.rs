...
macro_rules! full_featured {
    ...
    $($(#[$variant_meta:meta])* $variant_name:ident($field_name:path),)*
        ...
        impl std::convert::From<&$name> for waglayla_notify::events::EventType {
            fn from(value: &$name) -> Self {
                match value {
                    $($name::$variant_name(_) => waglayla_notify::events::EventType::$variant_name),*
                }
            }
        }

        impl std::convert::From<&$name> for waglayla_notify::scope::Scope {
            fn from(value: &$name) -> Self {
                match value {
                    $($name::$variant_name(_) => waglayla_notify::scope::Scope::$variant_name(waglayla_notify::scope::[<$variant_name Scope>]::default())),*
                }
            }
        }
