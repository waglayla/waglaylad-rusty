...
mod tests {
    ...
    use waglayla_consensus_core::{
        ...
        coinbase::MinerData,
        constants::{MAX_TX_IN_SEQUENCE_NUM, SOMPI_PER_WAGLAYLA, TX_VERSION},
    ...
    fn test_revalidate_high_priority_transactions() {
        ...
        let (parent_tx, child_tx_1) = create_parent_and_children_transactions(&consensus, vec![3000 * SOMPI_PER_WAGLAYLA]);
    ...
    fn create_transaction_with_utxo_entry(i: u32, block_daa_score: u64) -> MutableTransaction {
        ...
        let entry = UtxoEntry::new(SOMPI_PER_WAGLAYLA, script_public_key.clone(), block_daa_score, true);
        let output = TransactionOutput::new(SOMPI_PER_WAGLAYLA - DEFAULT_MINIMUM_RELAY_TRANSACTION_FEE, script_public_key);
    ...
    fn create_arrays_of_parent_and_children_transactions(
        ...
        (0..count)
            .map(|i| {
                create_parent_and_children_transactions(consensus, vec![500 * SOMPI_PER_WAGLAYLA, 3_000 * SOMPI_PER_WAGLAYLA + i as u64])
            })
            .unzip()
    }
    ...
    fn create_child_and_parent_txs_and_add_parent_to_consensus(consensus: &Arc<ConsensusMock>) -> Transaction {
        let parent_tx = create_transaction_without_input(vec![500 * SOMPI_PER_WAGLAYLA]);
