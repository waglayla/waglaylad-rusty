use crate::cli::WaglaylaCli;
use crate::imports::*;
use crate::result::Result;
use waglayla_bip32::{Language, Mnemonic, WordCount};
use waglayla_wallet_core::account::MULTISIG_ACCOUNT_KIND;
// use waglayla_wallet_core::runtime::wallet::AccountCreateArgsBip32;
// use waglayla_wallet_core::runtime::{PrvKeyDataArgs, PrvKeyDataCreateArgs};
// use waglayla_wallet_core::storage::AccountKind;
...
async fn create_multisig(ctx: &Arc<WaglaylaCli>, account_name: Option<String>, mnemonic_phrase_word_count: WordCount) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
