...
pub fn default_storage_folder() -> &'static str {
    ...
    // this function.
    unsafe { DEFAULT_STORAGE_FOLDER.get_or_insert("~/.waglayla".to_string()).as_str() }
}
...
pub fn default_wallet_file() -> &'static str {
    ...
    // however, the actual variable is accessible only through
    // this function.
    unsafe { DEFAULT_WALLET_FILE.get_or_insert("waglayla".to_string()).as_str() }
}
...
pub fn default_settings_file() -> &'static str {
    ...
    // however, the actual variable is accessible only through
    // this function.
    unsafe { DEFAULT_SETTINGS_FILE.get_or_insert("waglayla".to_string()).as_str() }
}
...
/// the storage folder will default to `~/.waglayla`
