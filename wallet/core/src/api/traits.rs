//!
//! API trait for interfacing with the Waglayla wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `waglayla`.
    ///
    ...
    /// applications such as KDX and waglaylanet.io web wallet using a deprecated derivation path
