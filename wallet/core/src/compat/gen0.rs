...
use waglayla_bip32::{ExtendedPrivateKey, Language, Mnemonic, Prefix, SecretKey};
use md5::Md5;
...
// ---
// {"type":"waglayla-wallet","encryption":"default","version":1,"generator":"pwa","wallet":{"mnemonic":"hex"}}
...
fn legacy_v0_keydata_location() -> Result<(PathBuf, Options)> {
    let filename = if runtime::is_windows() {
        ...
        fs::resolve_path(&format!("{appdata}/Waglayla/waglayla.kpk"))?
    } else if runtime::is_macos() {
        fs::resolve_path("~/Library/Application Support/Waglayla/waglayla.kpk")?
    } else {
        fs::resolve_path("~/.waglayla/waglayla.kpk")?
    };

    let options = workflow_store::fs::Options::with_local_storage_key("waglayla-wallet");
