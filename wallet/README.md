...
After starting the wallet shell (native of WASM) and starting a local waglayla Pyipad node (with `--testnet` and `--utxoindex`), you should perform the following actions:
```
network testnet
server localhost
connect
