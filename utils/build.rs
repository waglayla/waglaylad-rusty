...
fn main() {
    let success = if env::var("RUSTY_WAGLAYLA_NO_COMMIT_HASH").is_err() {
        if let Some(GitHead { head_path, head_ref_path, full_hash, short_hash }) = try_git_head() {
            ...
            println!("cargo:rustc-env=RUSTY_WAGLAYLA_GIT_FULL_COMMIT_HASH={full_hash}");
            println!("cargo:rustc-env=RUSTY_WAGLAYLA_GIT_SHORT_COMMIT_HASH={short_hash}");
            true
        } else {
            false
        }
    ...
    if !success {
        println!("cargo:rustc-env=RUSTY_WAGLAYLA_GIT_FULL_COMMIT_HASH=");
        println!("cargo:rustc-env=RUSTY_WAGLAYLA_GIT_SHORT_COMMIT_HASH=");
    }
}
