...
impl Default for WaglayladConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<WaglayladConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: WaglayladConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no waglaylad path is specified".to_string()));
        }
...
pub struct Waglaylad {
    inner: Arc<Mutex<Inner>>,
    mute: Arc<AtomicBool>,
    events: Channel<ProcessEvent>,
}

impl Default for Waglaylad {
    fn default() -> Self {
...
impl Waglaylad {
    pub fn new(args: WaglayladConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: WaglayladConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Waglayla node is already running.".to_string()));
            }
        }
        ...
        // let options = WaglayladOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Waglayla binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait WaglayladCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: WaglayladConfig) -> Result<()>;
