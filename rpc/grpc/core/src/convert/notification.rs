use crate::protowire::{
    waglaylad_response::Payload, BlockAddedNotificationMessage, WaglayladResponse, NewBlockTemplateNotificationMessage, RpcNotifyCommand,
};
use crate::protowire::{
...
use waglayla_rpc_core::{Notification, RpcError, RpcHash};
use std::str::FromStr;
use std::sync::Arc;
...
from!(item: &waglayla_rpc_core::Notification, WaglayladResponse, { Self { id: 0, payload: Some(item.into()) } });

from!(item: &waglayla_rpc_core::Notification, Payload, {
    match item {
...
from!(item: &waglayla_rpc_core::BlockAddedNotification, BlockAddedNotificationMessage, { Self { block: Some((&*item.block).into()) } });

from!(&waglayla_rpc_core::NewBlockTemplateNotification, NewBlockTemplateNotificationMessage);

from!(item: &waglayla_rpc_core::VirtualChainChangedNotification, VirtualChainChangedNotificationMessage, {
    Self {
...
from!(item: &waglayla_rpc_core::FinalityConflictNotification, FinalityConflictNotificationMessage, {
    Self { violating_block_hash: item.violating_block_hash.to_string() }
});

from!(item: &waglayla_rpc_core::FinalityConflictResolvedNotification, FinalityConflictResolvedNotificationMessage, {
    Self { finality_block_hash: item.finality_block_hash.to_string() }
});

from!(item: &waglayla_rpc_core::UtxosChangedNotification, UtxosChangedNotificationMessage, {
    Self {
...
from!(item: &waglayla_rpc_core::SinkBlueScoreChangedNotification, SinkBlueScoreChangedNotificationMessage, {
    Self { sink_blue_score: item.sink_blue_score }
});

from!(item: &waglayla_rpc_core::VirtualDaaScoreChangedNotification, VirtualDaaScoreChangedNotificationMessage, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

from!(&waglayla_rpc_core::PruningPointUtxoSetOverrideNotification, PruningPointUtxoSetOverrideNotificationMessage);
...
try_from!(item: &WaglayladResponse, waglayla_rpc_core::Notification, {
    item.payload
        .as_ref()
        .ok_or_else(|| RpcError::MissingRpcFieldError("WaglayladResponse".to_string(), "payload".to_string()))?
        .try_into()?
});

try_from!(item: &Payload, waglayla_rpc_core::Notification, {
    match item {
...
try_from!(item: &BlockAddedNotificationMessage, waglayla_rpc_core::BlockAddedNotification, {
    Self {
        block: Arc::new(
            item.block
                .as_ref()
...
try_from!(&NewBlockTemplateNotificationMessage, waglayla_rpc_core::NewBlockTemplateNotification);

try_from!(item: &VirtualChainChangedNotificationMessage, waglayla_rpc_core::VirtualChainChangedNotification, {
    Self {
...
try_from!(item: &FinalityConflictNotificationMessage, waglayla_rpc_core::FinalityConflictNotification, {
    Self { violating_block_hash: RpcHash::from_str(&item.violating_block_hash)? }
});

try_from!(item: &FinalityConflictResolvedNotificationMessage, waglayla_rpc_core::FinalityConflictResolvedNotification, {
    Self { finality_block_hash: RpcHash::from_str(&item.finality_block_hash)? }
});

try_from!(item: &UtxosChangedNotificationMessage, waglayla_rpc_core::UtxosChangedNotification, {
    Self {
...
try_from!(item: &SinkBlueScoreChangedNotificationMessage, waglayla_rpc_core::SinkBlueScoreChangedNotification, {
    Self { sink_blue_score: item.sink_blue_score }
});

try_from!(item: &VirtualDaaScoreChangedNotificationMessage, waglayla_rpc_core::VirtualDaaScoreChangedNotification, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

try_from!(&PruningPointUtxoSetOverrideNotificationMessage, waglayla_rpc_core::PruningPointUtxoSetOverrideNotification);
