...
pub enum GrpcServerError {
    ...
    ClosedHandler(WaglayladPayloadOps),
...
impl From<GrpcServerError> for waglayla_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => waglayla_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for waglayla_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => waglayla_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => waglayla_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        waglayla_notify::error::Error::ChannelSendError.into()
    }
}
