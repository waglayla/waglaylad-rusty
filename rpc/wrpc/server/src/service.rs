...
impl Default for Options {
    fn default() -> Self {
        Options { listen_address: "127.0.0.1:13110".to_owned(), verbose: false, grpc_proxy_address: None }
    }
}

/// ### WaglaylaRpcHandler
///
/// [`WaglaylaRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct WaglaylaRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl WaglaylaRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        WaglaylaRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for WaglaylaRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "waglayla" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<WaglaylaRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(WaglaylaRpcHandler::new(tasks, *encoding, core_service, options.clone()));
